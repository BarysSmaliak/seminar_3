...
Результатом выполнения команды станет вывод в окне терминала названия всех текущих веток репозитория. Та ветка, в которой мы находимся в настоящий момент, будет отмечена зелёным цветом.
...
Переход между двумя ветками осуществляется командой

    git checkout <branchname>

При этом в терминале появляется строка вида

    Switched to branch <'branchname'>

Важно отметить, что при переходе на новую ветку "голова" (HEAD) перемещается в ту версию репозитория, в которой эта новая ветка бфла создана. Т.е. более поздних коммитов мы не увидим.

### Слияние веток

Если мы готовы перелить информацию из второстепенных веток в *master*, необходимо воспользоваться командой 

    git merge <branchname>

__!!!ВАЖНО!__ В том случае, когда существует два варианта одной и той же информации в сливаемых ветках, Git выведет строку с сообщением о __конфликте__, и предложит оба варианта для того, чтобы выбор сделали Вы. 

После того, как выбор Вы сделаете, изменения в репозитории необходимо закоммитить.

### Удаление веток

Для того, чтобы удалить ненужную более ветку, необходимо воспользоваться командой 

    git branch -D <branchname>

Ветка будет удалена полностью, вместе со всеми коммитами и неслитыми изменениями в ней.

Как правило пользуются более "безопасным" выриантом удаления, при помощи команды

    git branch -d <branchname>

В этом случае ветка будет удалена только когда вся информация из неё слита. В противном случае Git удалению произойти не позволит.

### Просмотр дерева ветвления

Git позволяет визуализировать информацию о происходивших в репозитории ветвлениях. Это делается при помощи команды

    git log --graph

Параметр --graph используется для построения диаграммы из символов ASCII, отражающей структуру веток в истории коммитов. Этот параметр можно использовать в сочетании с параметром *--oneline*, который минимизирует выводимую на экран информацию, чем делает её более наглядной.

## Удаление репозитория

Удаление репозитория нужно ...
